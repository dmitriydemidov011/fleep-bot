...
async function loadUserData() {
    // Сначала грузим из localStorage (мгновенно, без задержки)
    const cached = DB.get('userData');
    if (cached) {
        userData = Object.assign(getDefaultUserData(), cached);
        updateBalance(); updateStats(); updateTasks();
        updateProfileInfo(); updateGameHistory(); updateCasesHistory();
    }

    // Потом грузим с сервера (актуальные данные со всех устройств)
    const userId = tg?.initDataUnsafe?.user?.id;
    if (userId) {
        try {
            const resp = await fetch(
                `${BACKEND_URL}/userdata/load?user_id=${userId}&init_data=${encodeURIComponent(tg?.initData||'')}`
            );
            if (resp.ok) {
                const result = await resp.json();
                if (result.data) {
                    const serverData = JSON.parse(result.data);
                    // Мержим: берём серверные данные как основу,
                    // но локальный прогресс игры тоже учитываем
                    const merged = Object.assign(getDefaultUserData(), serverData);
                    // Инвентарь: объединяем по id
                    const localInv  = (userData.inventory || []);
                    const serverInv = (serverData.inventory || []);
                    const invMap = {};
                    serverInv.forEach(g => { invMap[g.id] = g; });
                    localInv.forEach(g => { if (!invMap[g.id]) invMap[g.id] = g; });
                    merged.inventory = Object.values(invMap);
                    // История: объединяем
                    merged.gameHistory   = _mergeHistory(serverData.gameHistory,   userData.gameHistory);
                    merged.rocketHistory = _mergeHistory(serverData.rocketHistory, userData.rocketHistory);
                    merged.casesHistory  = _mergeHistory(serverData.casesHistory,  userData.casesHistory);
                    // Серебро: максимум
                    merged.balance.silver = Math.max(
                        serverData.balance?.silver || 0,
                        userData.balance?.silver   || 0
                    );
                    userData = merged;
            ...
            console.warn('load from server failed:', e.message);
        }
    }
    userData.lastVisit = new Date().toISOString();
    // НЕ вызываем saveUserData() здесь — избегаем перезаписи серверных данных старым кэшем
}

function _mergeHistory(serverArr, localArr) {
    const arr = [...(serverArr || []), ...(localArr || [])];
    const seen = new Set();
    return arr.filter(h => {
        const key = h.date || h.time || JSON.stringify(h);
...
function saveUserData() {
    // Всегда сохраняем локально как кэш
    DB.set('userData', userData);
...
async function _doSync() {
    ...
    if (!userId) return;
    try {
        await fetch(BACKEND_URL + '/userdata/save', {
            method: 'POST',
        ...
        // Сервер недоступен — данные в localStorage, потеря не страшна
        console.warn('sync to server failed:', e.message);
    }
}
