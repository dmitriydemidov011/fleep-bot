...
function endGameLose() {
    ...
    // Реф: считаем от проигрыша (ставки)
    _reportLossForRef(gameState.currentBet, gameState.betType);
    saveUserData();
    updateStats();
...
async function _reportLossForRef(amount, betType) {
    const userId = tg?.initDataUnsafe?.user?.id;
    if (!userId || amount <= 0) return;
    try {
        await fetch(BACKEND_URL + '/report_loss', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({user_id: userId, amount: amount,
                bet_type: betType, init_data: tg?.initData || ''})
        });
    } catch(e) {}
}
...
function selectCase(type) {
    ...
    checkCaseBalance();
    const balEl2 = document.getElementById('case-modal-balance');
    if (balEl2) {
        const sv = userData.balance.silver || 0;
        const gd = userData.balance.gold   || 0;
        balEl2.textContent = sv + ' F серебра  •  ' + gd + ' 🟡 золота';
    }
