...
def init_db():
    ...
    """)
    conn.execute("INSERT OR IGNORE INTO ref_settings(user_id, percent) VALUES(0, 10)")
    conn.commit()
    conn.close()
...
def save_user_data(user_id: int, data_json: str):
    conn = sqlite3.connect(DB_PATH)
    conn.execute(
...
async def http_userdata_save(request: web.Request) -> web.Response:
    if request.method == "OPTIONS":
        return web.Response(status=204, headers=CORS)
    try:
        ...
        import json as _j
        try:
            ud = _j.loads(data_json)
            g_s, s_s = get_balance(user_id)
            if "balance" not in ud: ud["balance"] = {}
            ud["balance"]["gold"] = g_s
            cs = int(ud["balance"].get("silver", 0))
            if cs > s_s: add_silver(user_id, cs - s_s); ud["balance"]["silver"] = cs
            else: ud["balance"]["silver"] = s_s
            data_json = _j.dumps(ud, ensure_ascii=False)
        except Exception: pass
        save_user_data(user_id, data_json)
        return web.json_response({"ok": True}, headers=CORS)
    except Exception as e:
        return web.json_response({"error": str(e)}, status=500, headers=CORS)

async def http_userdata_load(request: web.Request) -> web.Response:
    if request.method == "OPTIONS":
        return web.Response(status=204, headers=CORS)
    try:
        ...
        if not user_id:
            return web.json_response({"error": "no user_id"}, status=400, headers=CORS)
        data_json = load_user_data(user_id)
        gold, silver = get_balance(user_id)
        if data_json:
            import json as _j
            ud = _j.loads(data_json)
            if "balance" not in ud: ud["balance"] = {}
            ud["balance"]["gold"] = gold; ud["balance"]["silver"] = silver
            data_json = _j.dumps(ud, ensure_ascii=False)
        return web.json_response({"ok": True, "data": data_json, "gold": gold, "silver": silver}, headers=CORS)
    except Exception as e:
        return web.json_response({"error": str(e)}, status=500, headers=CORS)

def get_ref_percent(user_id: int) -> int:
    conn = sqlite3.connect(DB_PATH)
    row = conn.execute("SELECT percent FROM ref_settings WHERE user_id=?", (user_id,)).fetchone()
    if not row:
        row = conn.execute("SELECT percent FROM ref_settings WHERE user_id=0").fetchone()
    conn.close()
    return row[0] if row else 10

def set_ref_percent(user_id: int, percent: int):
    conn = sqlite3.connect(DB_PATH)
    conn.execute("INSERT INTO ref_settings(user_id, percent) VALUES(?,?) ON CONFLICT(user_id) DO UPDATE SET percent=excluded.percent", (user_id, percent))
    conn.commit(); conn.close()

def save_referral(inviter_id: int, invited_id: int):
    try:
        conn = sqlite3.connect(DB_PATH)
        conn.execute("INSERT OR IGNORE INTO referrals(inviter_id, invited_id) VALUES(?,?)", (inviter_id, invited_id))
        conn.commit(); conn.close()
    except Exception: pass

def pay_ref_bonus(invited_id: int, deposit_amount: int) -> int:
    try:
        conn = sqlite3.connect(DB_PATH)
        row = conn.execute("SELECT inviter_id FROM referrals WHERE invited_id=? AND bonus_paid=0", (invited_id,)).fetchone()
        conn.close()
        if not row: return 0
        inviter_id = row[0]
        bonus = max(1, round(deposit_amount * get_ref_percent(inviter_id) / 100))
        add_gold(inviter_id, bonus)
        conn2 = sqlite3.connect(DB_PATH)
        conn2.execute("UPDATE referrals SET bonus_paid=1 WHERE invited_id=?", (invited_id,))
        conn2.commit(); conn2.close()
        record_transaction(inviter_id, "", "", "referral_bonus", "ref", bonus, "gold")
        return bonus
...
def pay_ref_bonus_loss(player_id: int, loss_amount: int) -> int:
    try:
        conn = sqlite3.connect(DB_PATH)
        row = conn.execute("SELECT inviter_id FROM referrals WHERE invited_id=?", (player_id,)).fetchone()
        conn.close()
        if not row: return 0
        inviter_id = row[0]
        bonus = max(1, round(loss_amount * get_ref_percent(inviter_id) / 100))
        add_gold(inviter_id, bonus)
...
def get_ref_stats(user_id: int) -> dict:
    try:
        ...
        bonuses = conn.execute("SELECT COALESCE(SUM(amount),0) FROM transactions WHERE user_id=? AND type='referral_bonus'", (user_id,)).fetchone()[0]
        conn.close()
        return {"total": total, "bonuses": bonuses, "percent": get_ref_percent(user_id)}
    except Exception: return {"total": 0, "bonuses": 0, "percent": 10}

async def http_ref_stats(request: web.Request) -> web.Response:
    if request.method == "OPTIONS": return web.Response(status=204, headers=CORS)
    user_id = int(request.rel_url.query.get("user_id", 0))
    if not user_id: return web.json_response({"error": "no user_id"}, status=400, headers=CORS)
    return web.json_response(get_ref_stats(user_id), headers=CORS)
...
async def http_report_loss(request: web.Request) -> web.Response:
    ...
    try:
        ...
        if not user_id or amount <= 0: return web.json_response({"ok": False}, headers=CORS)
...
async def start_http(application):
    ...
    app_http.router.add_post("/userdata/save",    http_userdata_save)
    app_http.router.add_options("/userdata/save", http_userdata_save)
    app_http.router.add_get("/userdata/load",     http_userdata_load)
    app_http.router.add_options("/userdata/load", http_userdata_load)
    app_http.router.add_get("/ref_stats",         http_ref_stats)
    app_http.router.add_options("/ref_stats",     http_ref_stats)
    app_http.router.add_post("/report_loss",      http_report_loss)
    app_http.router.add_options("/report_loss",   http_report_loss)
...
async def admin(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🔗 Реферальная система",   callback_data="adm_refs")],
    ]
    await update.message.reply_text(
        "\n".join(summary_lines),
        parse_mode="Markdown",
...
async def admin_menu_choice(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "adm_broadcast":
        ...
        await query.message.reply_text("📢 Введи текст рассылки (или /cancel):")
        return WAIT_BROADCAST_TEXT

    if data == "adm_addbal":
        ctx.user_data["adm_state"] = "addbal"
        await query.message.reply_text(
    ...
    if data == "adm_stats":
        stats = get_transaction_stats()
        dep_s = dep_u = dep_t = wt = 0
        mc = {}
        for row in stats["summary"]:
            tt, meth, amt, cnt = row
            if tt == "deposit":
                dep_t += amt
                if meth == "stars": dep_s += amt
                elif meth == "usdt": dep_u += amt
            elif tt == "withdrawal":
                wt += amt
            mc[f"{tt}/{meth}"] = mc.get(f"{tt}/{meth}", 0) + cnt
        lines = ["📊 *Статистика*", "",
            f"⭐ Stars: *{dep_s}* ({mc.get('deposit/stars',0)} шт)",
            f"💵 USDT: *{dep_u}* ({mc.get('deposit/usdt',0)} шт)",
            f"📈 Итого: *{dep_t}*",
            f"📤 Выводов: *{wt}* ({mc.get('withdrawal/gift',0)} шт)",
            "", "📋 *Последние 20:*"]
        for row in stats["recent"]:
            ts, un, fn2, tt, meth, amt, cur = row
            who = f"@{un}" if un else (fn2 or "?")
            lines.append(f"{'⬆️' if tt=='deposit' else '⬇️'} {(ts or '')[:10]} {who}: {amt} {cur}/{meth}")
        await query.message.reply_text("\n".join(lines), parse_mode="Markdown")
        return ConversationHandler.END

    if data == "adm_gifts":
        rows = get_custom_gifts(active_only=False)
        lines = ["🎁 *Управление подарками*", ""]
        buttons = []
        if rows:
            for row in rows:
                gid, gname, gtype, gcost, gmin, gmax, gweight, gphoto = row
                lines.append(f"• *{gname}* — ⭐{gcost} | 🪙{gmin}–{gmax} {'🖼' if gphoto else '❌'}")
                buttons.append([InlineKeyboardButton(f"🗑 {gname}", callback_data=f"adm_gift_del_{gid}")])
        else:
            lines.append("_Подарков нет_")
        buttons.append([InlineKeyboardButton("➕ Добавить подарок", callback_data="adm_gift_add")])
        await query.message.reply_text(
            "\n".join(lines), parse_mode="Markdown",
    ...
    if data == "adm_gift_add":
        await query.message.reply_text(
            "🎁 *Добавление подарка*\n\nВведи название (например: Сердце):",
            parse_mode="Markdown"
        )
        return WAIT_GIFT_NAME

    if data.startswith("adm_gift_del_"):
        try:
            gid = int(data.replace("adm_gift_del_", ""))
            delete_custom_gift(gid)
            await query.message.reply_text(f"🗑 Подарок #{gid} удалён.")
        except Exception as e:
            await query.message.reply_text(f"❌ Ошибка: {e}")
        return ConversationHandler.END

    if data == "adm_refs":
        gp = get_ref_percent(0)
        try:
            conn_r = sqlite3.connect(DB_PATH)
            top = conn_r.execute(
                "SELECT r.inviter_id, u.username, COUNT(r.id) "
                "FROM referrals r LEFT JOIN users u ON u.user_id=r.inviter_id "
                "GROUP BY r.inviter_id ORDER BY COUNT(r.id) DESC LIMIT 10"
            ).fetchall()
            conn_r.close()
        except Exception:
            top = []
        lines = ["🔗 *Реферальная система*", "", f"🌐 Глобальный %: *{gp}%*", ""]
        for row in top:
            uid2, uname2, cnt2 = row[0], row[1], row[2]
            p2 = get_ref_percent(uid2)
            who2 = f"@{uname2}" if uname2 else str(uid2)
            lines.append(f"• {who2} — {cnt2} реф | {p2}%")
        kb2 = [
            [InlineKeyboardButton("🌐 Изменить глобальный %", callback_data="adm_ref_set_0")],
            [InlineKeyboardButton("👤 Персональный %", callback_data="adm_ref_set_user")],
        ]
        await query.message.reply_text(
            "\n".join(lines), parse_mode="Markdown",
    ...
    if data == "adm_ref_set_0":
        ctx.user_data["ref_target_id"] = 0
        gp2 = get_ref_percent(0)
        await query.message.reply_text(
            f"🌐 Текущий %: *{gp2}%*\n\nВведи новый процент (1–100):",
            parse_mode="Markdown"
        )
        return WAIT_REF_PERCENT

    if data == "adm_ref_set_user":
        await query.message.reply_text("👤 Введи username или ID пользователя:")
        return WAIT_REF_USER
...
async def addbal_receive_amount(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    _uid = ctx.user_data.get("addbal_user_id", 0)
    keyboard = [[
        InlineKeyboardButton("🟡 Золото",  callback_data=f"addbal_gold_{_uid}_{amount}"),
        InlineKeyboardButton("⚪ Серебро", callback_data=f"addbal_silver_{_uid}_{amount}"),
        InlineKeyboardButton("🟡+⚪ Оба",  callback_data=f"addbal_both_{_uid}_{amount}"),
    ]]
    await update.message.reply_text(
...
async def addbal_confirm_type(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    username  = _row[0] if _row and _row[0] else str(user_id)
    coin_type = f"addbal_{_ckey}"
...
async def run():
    ...
    # Глобальный handler — кнопки работают всегда
