...
async def addbal_confirm_type(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Декодируем из callback_data: addbal_gold_12345_500
    parts = query.data.split("_")
    # parts: ['addbal', 'gold', user_id, amount]
    try:
        coin_type_key = parts[1]          # gold / silver / both
        user_id       = int(parts[2])
        amount        = int(parts[3])
    except (IndexError, ValueError):
        # Fallback на старый способ (ctx.user_data)
        coin_type_key = query.data.replace("addbal_", "").split("_")[0]
        user_id  = ctx.user_data.get("addbal_user_id", 0)
        amount   = ctx.user_data.get("addbal_amount", 0)

    if not user_id or not amount:
        await query.message.reply_text("❌ Ошибка: данные потеряны. Начни заново: /admin")
        return ConversationHandler.END
    ...
    coin_type = f"addbal_{coin_type_key}"  # addbal_gold / addbal_silver / addbal_both
...
async def run():
    ...
    admin_conv = ConversationHandler(
        ...
        states={
            ...
            WAIT_ADDBAL_TYPE:    [CallbackQueryHandler(addbal_confirm_type, pattern=r"^addbal_(gold|silver|both)(_\d+_\d+)?$")],
