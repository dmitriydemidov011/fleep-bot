...
def set_ref_percent(user_id: int, percent: int):
    conn = sqlite3.connect(DB_PATH)
    conn.execute(
        "INSERT INTO ref_settings(user_id, percent) VALUES(?,?) "
...
def pay_ref_bonus(invited_id: int, deposit_amount: int) -> int:
    conn = sqlite3.connect(DB_PATH)
    row = conn.execute(
        "SELECT inviter_id FROM referrals WHERE invited_id=? AND bonus_paid=0",
        (invited_id,)
    ).fetchone()
    conn.close()
    if not row:
        return 0
    inviter_id = row[0]
    percent = get_ref_percent(inviter_id)
    bonus = max(1, round(deposit_amount * percent / 100))
...
def get_ref_stats(user_id: int) -> dict:
    ...
    bonuses = conn.execute(
        "SELECT COALESCE(SUM(amount),0) FROM transactions WHERE user_id=? AND type='referral_bonus'",
        (user_id,)
    ).fetchone()[0]
    conn.close()
...
async def http_ref_stats(request: web.Request) -> web.Response:
    """GET /ref_stats?user_id=... — статистика рефералов пользователя"""
    if request.method == "OPTIONS":
        return web.Response(status=204, headers=CORS)
...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Обрабатываем реф ссылку: /start ref_12345
    args = ctx.args or []
    if args and args[0].startswith("ref_"):
        try:
            inviter_id = int(args[0].replace("ref_", ""))
            if inviter_id != user.id:
                save_referral(inviter_id, user.id)
                logger.info(f"New referral: inviter={inviter_id} invited={user.id}")
                ...
                try:
                    ...
                    await ctx.bot.send_message(
                        ...
                        f"Ты получишь *{percent}%* от его первого пополнения 🟡",
                        parse_mode="Markdown"
                    )
                except Exception:
                    pass
...
async def successful_payment(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Начисляем реф бонус пригласителю (только при первом пополнении)
    ref_bonus = pay_ref_bonus(user.id, coins)
    if ref_bonus > 0:
        logger.info(f"Ref bonus paid: {ref_bonus} gold for inviter of user {user.id}")
...
async def receive_ref_user(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Получаем пользователя для персонального % рефки"""
    uname = update.message.text.strip().lstrip("@")
    row = find_user_by_username(uname)
    if not row:
        await update.message.reply_text(f"❌ Пользователь @{uname} не найден. Попробуй ещё:")
        return WAIT_REF_USER
    ctx.user_data["ref_target_id"] = row[0]
    current_pct = get_ref_percent(row[0])
    await update.message.reply_text(
        f"✅ @{row[1] or uname}\n"
        f"Текущий процент: *{current_pct}%*\n\n"
        f"Введи новый процент (1–100):",
        parse_mode="Markdown"
    )
    return WAIT_REF_PERCENT


async def receive_ref_percent(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Устанавливаем процент рефки"""
    try:
        ...
        return WAIT_REF_PERCENT
    target_id = ctx.user_data.get("ref_target_id", 0)
    set_ref_percent(target_id, pct)
    scope = "глобальный" if target_id == 0 else f"для ID {target_id}"
    await update.message.reply_text(
        f"✅ Реф процент ({scope}) установлен: *{pct}%*",
        parse_mode="Markdown"
    )
    return ConversationHandler.END
...
async def admin(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🔗 Реф система", callback_data="adm_refs")],
    ]
    await update.message.reply_text(
        "\n".join(summary_lines),
        parse_mode="Markdown",
...
async def admin_menu_choice(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "adm_refs":
        ...
        conn3.close()
        lines = ["🔗 *Реф система*", "", f"🌐 Глобальный %: *{gp}%*", ""]
        for uid, uname, cnt in top:
            p = get_ref_percent(uid)
            who = f"@{uname}" if uname else str(uid)
            lines.append(f"• {who} — {cnt} реф | {p}%")
        kb = [
