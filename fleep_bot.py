...
async def admin_menu_choice(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    data  = query.data
    logger.info(f"Admin button: {data} from {query.from_user.username}")
    ...
    if data == "adm_addbal":
        ctx.user_data["admin_mode"] = "addbal"
        await query.message.reply_text(
    ...
    if data == "adm_stats":
        ...
        for row in stats["recent"]:
            ...
            icon = "⬆️" if tt == "deposit" else "⬇️"
            lines.append(f"{icon} {(ts or '')[:10]} {who}: {amt} {cur} / {meth}")
    ...
    if data == "adm_gifts":
        ...
        if rows:
            for row in rows:
                ...
                mark = "🖼" if gphoto else "❌"
                lines.append(f"• *{gname}* — ⭐{gcost} | 🪙{gmin}–{gmax} {mark}")
                buttons.append([InlineKeyboardButton(
                    f"🗑 Удалить: {gname}", callback_data=f"adm_gift_del_{gid}"
                )])
        else:
...
async def run():
    ...
    # ── Глобальный handler для кнопок админки ──
    # Работает всегда, независимо от состояния ConversationHandler
    ...
    async with app:
        ...
        async def error_handler(upd, ctx):
            logger.error(f"Update {upd} caused error: {ctx.error}", exc_info=ctx.error)
