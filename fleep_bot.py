...
async def admin_menu_choice(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Пополнить баланс ──
    if data == "adm_addbal":
        ctx.user_data["admin_state"] = "addbal"
        await query.message.reply_text(
            "💰 *Пополнение баланса*\n\nВведи username пользователя (с @ или без):",
            parse_mode="Markdown"
        )
    ...
    if data == "adm_stats":
        stats = get_transaction_stats()
        dep_stars = dep_usdt = dep_total = with_total = 0
        method_counts = {}
        for row in stats["summary"]:
            ...
            if ttype == "deposit":
                ...
                with_total += amount
            method_counts[f"{ttype}/{method}"] = method_counts.get(f"{ttype}/{method}", 0) + cnt
        lines = [
            "📊 *Полная статистика*", "",
            f"⭐ Stars: *{dep_stars} 🟡* ({method_counts.get('deposit/stars', 0)} шт)",
            f"💵 USDT: *{dep_usdt} 🟡* ({method_counts.get('deposit/usdt', 0)} шт)",
            f"📈 Итого: *{dep_total} 🟡*",
            f"📤 Выводов: *{with_total}* ({method_counts.get('withdrawal/gift', 0)} шт)",
            "", "📋 *Последние 20:*",
        ]
        for row in stats["recent"]:
            ts, uname, fname, ttype, method, amount, currency = row
            who = f"@{uname}" if uname else (fname or "?")
            icon = "⬆️" if ttype == "deposit" else "⬇️"
            lines.append(f"{icon} {ts[:10]} {who}: {amount} {currency} via {method}")
        await query.message.reply_text("\n".join(lines), parse_mode="Markdown")
        return ConversationHandler.END

    # ── Управление подарками ──
    if data == "adm_gifts":
        existing = get_custom_gifts(active_only=False)
        ...
        if existing:
            for row in existing:
                ...
                lines.append(f"• *{gname}* — ⭐{gcost} | 🪙{gmin}–{gmax} | {photo_mark}")
                buttons.append([InlineKeyboardButton(
                    f"🗑 {gname}", callback_data=f"adm_gift_del_{gid}"
                )])
        else:
    ...
    if data.startswith("adm_gift_del_"):
        try:
            gift_id = int(data.replace("adm_gift_del_", ""))
            delete_custom_gift(gift_id)
            await query.message.reply_text(f"🗑 Подарок #{gift_id} удалён.")
        except Exception as e:
...
async def run():
    ...
    # ─── Отдельный глобальный handler для кнопок админки ─────────────────────────
# (НЕ внутри ConversationHandler — чтобы работал всегда независимо от состояния)
    app.add_handler(CallbackQueryHandler(
        admin_menu_choice,
        pattern=r"^adm_(broadcast|addbal|stats|gifts|gift_add|gift_del_.+)$"
    ))
    app.add_handler(CallbackQueryHandler(
        addbal_confirm_type,
        pattern=r"^addbal_(gold|silver|both)$"
    ))

    # ConversationHandler только для текстового ввода
    ...
    states={
        ...
        WAIT_BROADCAST_BTN: [
            MessageHandler(filters.TEXT & ~filters.COMMAND, receive_broadcast_btn),
        ],
        WAIT_ADDBAL_USER: [
            MessageHandler(filters.TEXT & ~filters.COMMAND, addbal_receive_user),
        ],
        WAIT_ADDBAL_AMOUNT: [
            MessageHandler(filters.TEXT & ~filters.COMMAND, addbal_receive_amount),
        ],
        WAIT_ADDBAL_TYPE: [
            CallbackQueryHandler(addbal_confirm_type, pattern=r"^addbal_(gold|silver|both)$"),
        ],
        WAIT_GIFT_NAME: [
            MessageHandler(filters.TEXT & ~filters.COMMAND, receive_gift_name),
        ],
        WAIT_GIFT_COST: [
            MessageHandler(filters.TEXT & ~filters.COMMAND, receive_gift_cost),
        ],
        WAIT_GIFT_COINS: [
            MessageHandler(filters.TEXT & ~filters.COMMAND, receive_gift_coins),
        ],
        WAIT_GIFT_PHOTO: [
