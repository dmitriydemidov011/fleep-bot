...
async def http_userdata_save(request: web.Request) -> web.Response:
    ...
    try:
        ...
        if not user_id:
            return web.json_response({"error": "no user_id"}, status=400, headers=CORS)
        # Синхронизируем gold с сервером
        import json as _j
        try:
            ud = _j.loads(data_json)
            g_srv, s_srv = get_balance(user_id)
            if "balance" not in ud:
                ud["balance"] = {}
            ud["balance"]["gold"] = g_srv
            client_s = int(ud["balance"].get("silver", 0))
            if client_s > s_srv:
                add_silver(user_id, client_s - s_srv)
                ud["balance"]["silver"] = client_s
            else:
                ud["balance"]["silver"] = s_srv
            data_json = _j.dumps(ud, ensure_ascii=False)
        except Exception as e:
            logger.warning(f"userdata merge: {e}")
...
async def http_userdata_load(request: web.Request) -> web.Response:
    ...
    try:
        ...
        if data_json:
            ...
            if "balance" not in ud:
                ud["balance"] = {}
            ud["balance"]["gold"]   = gold
            ud["balance"]["silver"] = silver
            data_json = _j.dumps(ud, ensure_ascii=False)
...
def pay_ref_bonus_loss(player_id: int, loss_amount: int) -> int:
    """Реф бонус от проигрыша — каждый раз."""
    try:
        conn = sqlite3.connect(DB_PATH)
        row = conn.execute(
...
def get_ref_stats(user_id: int) -> dict:
    try:
        ...
        return {"total": 0, "bonuses": 0, "percent": 10}
