...
def pay_ref_bonus_loss(player_id: int, loss_amount: int) -> int:
    """Реф бонус от проигрыша — начисляется каждый раз"""
    try:
        conn = sqlite3.connect(DB_PATH)
        row = conn.execute(
...
async def http_report_loss(request: web.Request) -> web.Response:
    """POST /report_loss — считает реф бонус от проигрыша игрока"""
    if request.method == "OPTIONS":
        return web.Response(status=204, headers=CORS)
    try:
        data    = await request.json()
...
async def admin_menu_choice(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    data = query.data
    logger.info(f"Admin CBQ: {data} from @{query.from_user.username}")

    if data == "adm_broadcast":
        ctx.user_data["adm_state"] = "broadcast"
        await query.message.reply_text(
            "📢 *Рассылка*\n\nВведи текст сообщения (или /cancel):",
            parse_mode="Markdown"
        )
        return WAIT_BROADCAST_TEXT
    ...
    if data == "adm_refs":
        gp = get_ref_percent(0) if "get_ref_percent" in dir() or "get_ref_percent" in globals() else 10
        try:
        ...
        for row in top:
            uid, uname, cnt = row[0], row[1], row[2]
            p = get_ref_percent(uid) if callable(get_ref_percent) else gp
        ...
        kb = [
            ...
            [InlineKeyboardButton("👤 Персональный % пользователю", callback_data="adm_ref_set_user")],
        ]
        await query.message.reply_text(
            "\n".join(lines), parse_mode="Markdown",
...
async def addbal_confirm_type(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        coin_key = parts[1] if len(parts) > 1 else "gold"
    if not user_id or not amount:
        await query.message.reply_text("❌ Данные потеряны. Начни заново: /admin")
        return ConversationHandler.END
    conn_u = sqlite3.connect(DB_PATH)
    row_u  = conn_u.execute("SELECT username FROM users WHERE user_id=?", (user_id,)).fetchone()
    conn_u.close()
...
async def run():
    ...
    # Глобальный — ловит кнопки после завершения ConvHandler
