...
async def http_userdata_save(request: web.Request) -> web.Response:
    """POST /userdata/save — сохраняет userData клиента"""
    if request.method == "OPTIONS":
        return web.Response(status=204, headers=CORS)
    try:
        ...
        # Синхронизируем gold/silver из серверных данных (они приоритетнее)
        import json as _json
        try:
            ud = _json.loads(data_json)
            gold_server, silver_server = get_balance(user_id)
            if "balance" not in ud:
                ud["balance"] = {}
            # Серверное золото всегда приоритет
            ud["balance"]["gold"] = gold_server
            # Серебро берём максимум (клиент мог заработать в игре)
            client_silver = int(ud["balance"].get("silver", 0))
            if client_silver > silver_server:
                add_silver(user_id, client_silver - silver_server)
                ud["balance"]["silver"] = client_silver
            else:
                ud["balance"]["silver"] = silver_server
            data_json = _json.dumps(ud, ensure_ascii=False)
        except Exception as e:
            logger.warning(f"userdata merge error: {e}")

        save_user_data(user_id, data_json)
        return web.json_response({"ok": True}, headers=CORS)
    except Exception as e:
        logger.error(f"userdata_save error: {e}")
        return web.json_response({"error": str(e)}, status=500, headers=CORS)


async def http_userdata_load(request: web.Request) -> web.Response:
    """GET /userdata/load?user_id=... — загружает userData клиента"""
    if request.method == "OPTIONS":
        return web.Response(status=204, headers=CORS)
    try:
        ...
        if data_json:
            import json as _json
            ud = _json.loads(data_json)
            # Всегда подставляем серверный gold
            if "balance" not in ud:
                ud["balance"] = {}
        ...
        logger.error(f"userdata_load error: {e}")
        return web.json_response({"error": str(e)}, status=500, headers=CORS)
...
async def receive_gift_name(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    gift_type = re.sub(r"[^a-z0-9_]", "", slug.replace(" ", "_"))[:20]
    if not gift_type:
        gift_type = f"gift_{len(_gift_draft)+1}"
...
async def admin_menu_choice(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "adm_gift_add":
        await query.message.reply_text(
            "🎁 *Добавление подарка*\n\nВведи название подарка (например: Сердце):",
            parse_mode="Markdown"
        )
...
async def run():
    ...
    admin_conv = ConversationHandler(
        entry_points=[
            ...
            CallbackQueryHandler(admin_menu_choice, pattern=r"^adm_(broadcast|addbal|stats|gifts|gift_add|gift_del_.+)$"),
        ],
        states={
            WAIT_BROADCAST_TEXT: [
                CallbackQueryHandler(admin_menu_choice, pattern=r"^adm_(broadcast|addbal|stats|gifts|gift_add|gift_del_.+)$"),
            ...
            WAIT_GIFT_NAME:  [
                MessageHandler(filters.TEXT & ~filters.COMMAND, receive_gift_name),
                CallbackQueryHandler(admin_menu_choice, pattern=r"^adm_(broadcast|addbal|stats|gifts|gift_add|gift_del_.+)$"),
            ],
